...
## RStudio 
RStudio is the free and open-source IDE (Integrated Desktop Environment) that is used by 99.99% of R users. It runs R inside of it and has lots of helpful bells and whistles that make using R much easier. RStudio is one of the reasons for the success of R.

Although they are a for-profit company, [RStudio is also Public Benefit Corporation which means they have legally-binding altruistic goals](https://www.rstudio.com/about/what-makes-rstudio-different/) including keeping RStudio free for individual users. [They make money selling and supporting enhanced versions of their free products for industry](https://rviews.rstudio.com/2016/10/12/interview-with-j-j-allaire/), which is a common open-source software business model. Thus, there is no rational basis to fear that, after investing your time in learning R in RStudio, you might have it taken away or somehow be forced to pay for it. 

+ RStudio GUI is actually is rendering a web page.  Thus, it looks and works the same across platforms (Windows, MacOS, and Linux) and you could remotely access a powerful RStudio server through your web browser.
...
## R Packages 
R Packages allow R users to easily share functions and data sets with each other. [CRAN](https://cran.r-project.org/), the primary website for downloading R packages currently has [> 18k](https://cran.r-project.org/web/packages/available_packages_by_name.html). R packages are most commonly used to share functions, but they are also useful for data distribution. The main goal of NCIFD is to clean-up and distribute NCWRC data sets among NCWRC staff, but it also stores some functions you may find useful.

# What can a Division R Package Do for Us?
## Data Sets
IFD has many relatively small data sets that are relevant outside of a single district but they can be difficult to find because there is no single source for their distribution. In addition, they are typically difficult to use because they are not arranged neatly in a single 2-dimensional data set and/or are very messy. For examples:

+ Available through PAWS but exported data is very messy
...
+ Fisheries Research with no home on PAWS 
...
+ No Standardized Distribution 
  ...
  + Waterbody Codes from Fish (1968) - only available in print 
  + Coldwater Stocking Coordinates - spreadsheet on Scott Marsh's computer
  + County Codes - hatchery version of these codes are on a PDF scan of an old piece of paper
  + Missing Reports - spreadsheet on Powell's computer
  + Raleigh Library Contents - spreadsheet on Shauna Glover's computer
  + Fish Species codes - no single source, PDF scans from throughout the agency, some on PAWS, inconsistencies among sources

## Functions
We can make R functions to solve IFD problems (like cleaning-up the output from BIODE queries) or share our homebrewed functions inside the Divison.

# NCIFD Introduction
NCIFD is an R package for the Inland Fisheries Division (IFD) of the North Carolina Wildlife Resources Commission. It contains functions and data sets authored by the staff. The first non-beta version Aggressive Alosid version 1.0.0 was released on 2022-02-08. A 'patch' version 1.0.1 was released recently. Minor releases with new and updated functions and data sets are expected every July (after the spring stockings), November (after the normal fall stockings), and February (after the new MTLSs and WWSLs are finalized).

## Install NCIFD
NCIFD is distributed from a MS Teams group for IFD R-users. E-mail [Powell Wheeler](mailto:powell.wheeler@ncwildlife.org) if you want access to the shared folder. There are 5 files available after each release: 

1. NCIFD_source.zip contains the source code. Its a zip-compressed directory structure that holds all the files that make the package. 
2. IntroToNCIFD.pdf is a general overview of the package with installation procedures.
...
# NCIFD Contents
## Data Sets
The versions in the package have has substantial clean-up and are instantly available in an R session. They are updated with the most recent data when new versions of the package are released.  
...
+ Research Resources
    ...
    + missingReports. A list of NCWRC publications that are not stored on PAWS. Many are currently lost.
    + raleighLibrary. Inventory of the IFD and WMD libraries in the Raleigh Office.
...
+ Fishing
    + ncarp. NCARP awards data base records.
...
# Using Package Data Sets
## Browsing a data set in R
It's difficult to browse through data in R. This is often bewildering for those coming to R from Excel. Here are several ways to view your data:
...
head(wwsl) #only show top 6 rows. Notice it overflows the screen in this document.
```
...
list.files('~/Downloads', 'trout.+csv', full.names = TRUE)
```
4). NCIFD also has three functions that help browse data: dfScan(), dfCols(), dfSlim(), and ferret(). We'll look at those with the package functions. 
...
The data sets in {NCIFD} often integrate information from each other. For example, 'warmaterStockingTrips' contains numeric codes for county names and uses 'counties' to decode them (e.g., 944 = Haywood). In addition, it uses 'fishes' to decode species codes into species names and 'waterbodies' and 'wwsl' to convert waterbody codes into waterbody names (Figure 1).
...
+ Use coldwaterStockingTrips as an example
+ NCIFD/data-raw
...
## Functions
Some of the functions have specific IDF uses such as cleanBIODE(), relativeWeight(), standardWeight() and others were created to help with specific research projects. In addition, there are other functions (that are not listed here) which are used to help build the package.
...
+ NCIFD Development (Figure 2)
  ...
  + stringCleaning() corrects a universe of bad formatting often found in notes columns and decodes > 100 common IFD abbreviations.
...
## cleanBIODE()
Cleans-up EXCEL files exported from BIODE queries.
...
## standardWeight()
Calculates standard weight of 47 NC fishes
...
+ references in help(standardWeight)
...
+ you can skip calculating Ws and get Wr directly
+ uses standardWeight()
+ alias: wr()
+ tested against published and known values - NCIFD/tests/testthat/test_condition.R
...
## Z2A()
Quickly convert an instantaneous mortality rate (Z) to an annual mortality rate (A). Also, converts the standard error and bootstraps confidence intervals with the gamma distribution (Bolker 2008).
...
NCIFD contains several data sets, most of which are 2-dimensional data.frames. Because viewing data in R can be difficult, the package contains three functions that can help with that.

1. dfCols(): shows a summary of what is contained in the data.frame columns.

```{r dfCols, eval=TRUE, message=FALSE, warning=FALSE, include=TRUE}
dfCols(zipCodeCoords) #no SE specified
```
...
dfScan(afsFishes, 10) #no SE specified
```

3. dfSlim(): drops the right-side columns that won't fit on your screen to prevent wrap-around.

```{r dfSlim, eval=TRUE, message=FALSE, warning=FALSE, include=TRUE}
dfSlim(head(coldwaterStockingTrips)) #no SE specified
```


## ferret()
Searches data.frames for strings and returns the rows that have matches. It is designed to quickly pull up information in NCIFD data.frames. With some pacakge familiarity and practice, you can use it while talking to anglers on the phone. By default, it finds case-insnesitive partial matches. For example, searching for 'WY' will match 'wy' and 'hwy'. However, if exact=TRUE, 'WY' will only match 'WY'. Here's some examples of how I've used it:

1. Is Charles D Owen still getting winter pond stockings?
```{r ferret01, eval=FALSE, message=FALSE, warning=FALSE, include=TRUE}
ferret(coldwaterStockingTrips, 'charles')
```

2. You jerks only stocked Big Snowbird HS once all last year.
...
3. When are Striped Bass typically stocked in Lake Hiwassee?
```{r ferret03, eval=FALSE, message=FALSE, warning=FALSE, include=TRUE}
ferret(warmwaterStockingTrips, c('sb', 'hiwassee')) #search for multiple strings at once with c()
```

4. Find all of David Yow’s Creel Reports
```{r ferret04, eval=FALSE, message=FALSE, warning=FALSE, include=TRUE}
ferret(reports, c('Yow', 'creel')) #search for multiple strings at once with c()
```

5. What months are Bear, Cedar Cliff, Wolf and Tanassee Stocked?
...
6. What data do you have from Calderwood, Santeetlah, Cheoah, and Emory reservoirs?
```{r ferret06, eval=FALSE, message=FALSE, warning=FALSE, include=TRUE}
ferret(reports, 'Calderwood|Santeetlah|Cheoah|Emory') #do an OR search with pipe symbol |; only works with exact=FALSE
```

7. When are Walleye normally stocked?
...
## ordinalDate()
Convert dates into ordinal dates. For example, "2008-01-01" becomes 1 and "2000-01-02" becomes 2. This is useful for creating a common Jan-Dec x-axis when plotting multiple years of a time-series. R date format returns integers and POSIXct dates return decimal numbers. 
...
```
![Daily flows on the Davidson River showing hurricane flows. ordinalDate() was used to create a common (1-365) x-axis for each year before the axis labels (months) were overlain.](pic/allFlow.png)
