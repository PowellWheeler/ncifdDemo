...
## RStudio 
RStudio is a company that makes a free and open-source program for R which is also called RStudio. RStudio (program) is an Integrated Desktop Environment that runs R inside of it and has many helpful features that make R easier to use and is used by 99.99% of R users. Rstudio (company) also produces a series of packages for R that are collectively known as the 'tidyverse' which also make R easier to use. RStudio and its products are s one of the reasons for the success of R.
