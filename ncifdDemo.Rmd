...
## R
[R](https://en.wikipedia.org/wiki/R_(programming_language)) is the [free](https://en.wikipedia.org/wiki/Free_software) and [open-source](https://en.wikipedia.org/wiki/Open_source) programming language has become a standard data analysis tool, not just in science, but across disciplines. R uses hardware very efficiently and runs well on old and low-powered computers. 

## RStudio 
RStudio is the free and open-source IDE (Integrated Desktop Environment) that is used by 99.99% of R users. It runs R inside of it and many helpful features that make R easier to use. RStudio is, undoubtedly, one of the reasons for the success of R.
...
path.package('NCIFD') #NCIFD is a directory on your hdd. library() allows R to access the directory.
```
...
+ Research Resources
  ...
  + reports; contents of the PAWS database of IFDivision publications.
+ Research Projects
...
The data sets in {NCIFD} often aggregate information from each other (Figure 1). For example, 'warmaterStockingTrips' contains numeric codes for counties and uses the 'counties' data set to decode them (e.g., 944 = Haywood). 'counties' is also used to assign district numbers to the trips (Haywood = 9). In addition, 'warmaterStockingTrips' uses 'fishes' to decode species codes into species names and 'waterbodies' and 'wwsl' to convert waterbody codes into waterbody names. R's ability to merge data from other data sets is more powerful than lookup functions in Excel and less powerful than relational databases.
