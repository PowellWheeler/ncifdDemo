...
## standardWeight() and relativeWeight()
Calculate condition for 47 NC fishes
+ relativeWeight() uses standardWeight()
+ alises:  ws(), wr() 
...
ferret(mtsl, 'bear|cedar cliff|wolf|tanassee') #do an OR search with pipe symbol | 
```
...
ferret(reports, 'Calderwood|Santeetlah|Cheoah|Emory') #do an OR search with pipe symbol | 
```
...
#try exact matching
ferret(warmwaterStockingTrips, c('WY'), exact = TRUE) #notice case sensitive
```
