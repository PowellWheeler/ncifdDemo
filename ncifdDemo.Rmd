...
output:
 pdf_document:
  ...
  #number_sections: yes
#LATEX options
geometry: "left=0.5in,right=0.5in,top=0.75in,bottom=0.752in"
fontsize: 10pt
urlcolor: blue
---
...
**This document** was written in RStudio (`r RStudio_ver`) in Rmarkdown (`r rmarkdown_ver`) and rendered at `r format(Sys.time(),"%B %d, %Y at %H:%M")` using on R (`r R_ver`), NCIFD (`r NCIFD_ver`), and knitr (`r knitr_ver`).
...
## R
[R](https://en.wikipedia.org/wiki/R_(programming_language)) is the [free](https://en.wikipedia.org/wiki/Free_software) and [open-source](https://en.wikipedia.org/wiki/Open_source) programming language that is becoming the standard tool for data analysis, not just in science, but across disciplines. R uses hardware very efficiently and runs well on old and low-powered computers. 
...
+ Dark themes are available. tools -> global options -> appearance -> Editor Theme. I use Ambiance or Sky.
...
+ Up and Down Arrows access command history in the console.
+ If you haven't upgraded your computer screen in a while now is a good time. 1080p FHD screens are old (> 8 y) technology and the extra screen real estate will help with RStudio.
...
+ Data from on-going fisheries research projects have no home on PAWS. 
   + Black Bass Genetics - results spreadsheet format changes constantly.
...
{NCIFD} is an R package for the Inland Fisheries Division of the North Carolina Wildlife Resources Commission (NCWRC). The main goal of {NCIFD} is to increase the availability and usefulness of Agency and Division data set among IFD staff, but it stores some functions also. The first non-beta version 1.0.0 'Aggressive Alosid' was released on 2022-02-08 and a 'patch' version 1.0.1 was released recently. Releases with new and updated functions and data sets are expected three times per year at natural breaks in data collection: 1) July (after the spring stockings), 2) November (after the normal fall stockings), and 3) February (after the new MTLSs and WWSLs are finalized).
...
## Getting Help
{NCIFD} has package-wide help that includes information about the project, as well as all the data sets and functions in the package. In RStudio, you can click links to view the specific help information for all the functions and data sets and there are hyperlinks to internet resources. Everything in the package has useful help information.
...
## Browsing Data in R
Browse data is difficult in R, which can be disorientating for those coming from Excel. Here are several ways to view data:
...
head(wwsl) #only shows top 6 rows. Notice it overflows the screen and wraps around.
```
...
```
4). {NCIFD} also has four functions that help browse data: dfScan(), dfCols(), dfSlim(), and ferret(). See [Data.frame Viewing Functions] below. 
...
The data sets in {NCIFD} often aggregate information from each other. For example, 'warmaterStockingTrips' contains numeric codes for counties and uses the 'counties' data set to decode them (e.g., 944 = Haywood). 'counties' is also used to assign District numbers to the trips (Haywood = 9). In addition, 'warmaterStockingTrips' uses 'fishes' to decode species codes into species names and 'waterbodies' and 'wwsl' to convert waterbody codes into waterbody names (Figure 1). R's ability to merge data from other data sets is more powerful than lookup functions in Excel and less powerful than relational databases.
...
Example: coldwaterStockingTrips compoents in the source code.
...
Demonstrate: automation/reproducibility with coldwaterStockingTrips
...
# {NCIFD} Functions
{NCIFD} contains a variety of functions. Some of the functions have specific IFD uses such as cleanBIODE(), relativeWeight(), standardWeight(), others were created to help with specific research projects, and others help build the package.
...
+ Console Tools
  ...
  + fruitSalad(); creates a data.frame with X rows of randomized information about fictitious salads for testing tricky R code. 
...
+ Functions for {NCIFD} Development
  ...
  + stringCleaning() corrects a universe of bad formatting often found in 'notes' and 'comments' columns and decodes > 100 common IFD abbreviations.
...
+ Species equation parameters are hidden data.frame: NCIFD:::wsLookup.
 + Some species have multiple equations.
  + Defaults to most useful in NC (BKT = SE BKT) or most general (MKY = both sexes).
+ AFS-stylized references for standard weight equations are in help(standardWeight).
...
relativeWeight('BKT', 150, 32.7, eq='B') #eq='B' selects Hyatt and Hubert (2001)
